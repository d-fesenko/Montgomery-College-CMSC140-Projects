    ...
    // Write your code here:
	   ...
	   for(int i = 0; i < intArray.length; i++) {
		   if(intArray[i][column] < minimum) {
			   ...
			   animate(i, column, i); 
			   //When I used animate(i, column) it gave me an error "wrong number of arguments to animate method." Using (i, column, i) worked for some reason.
		   }  
	   }
	   ...
	   for(int i = 0; i < intArray.length; i++) {
		   for(int j = 0; j < intArray[i].length; j++) {
			   if(intArray[i][j] == value) {
				   count++;
				   animate(i, j, i);
				   //Again animate(i, j) did not work, but animate(i, j, i) worked... not sure why
			   }
		   }
	   }
	   return count;
